...
GeometryPass::GeometryPass(Graphics& gfx) 
{
    device = gfx.GetDevice().Get();
    context = gfx.GetContext().Get();
    width = gfx.GetWidth();
...
void GeometryPass::CreateTexture()
{
...
void GeometryPass::CreateFullscreenQuad()
{
    //获取Graphics上资源
...
void GeometryPass::PutPixel(int x, int y, unsigned int color)
{
...
void GeometryPass::Resize(Graphics& gfx)
{
    if (device != nullptr) {
        width = gfx.GetWidth();
        height = gfx.GetHeight();
